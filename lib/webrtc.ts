...
export class WebRTCManager {
  ...
  async replaceVideoTrack(track: MediaStreamTrack | null) {
    // Guarde a faixa atual também para participantes que entrarem depois do início
    // do compartilhamento.
    ...
    for (const { connection } of this.peers.values()) {
      ...
      // Uma sala pode ter sido criada sem câmera. Reserve uma faixa de vídeo
      // negociada para que o compartilhamento ainda possa ser enviado.
  ...
  private ensurePeerConnection(peerId: string): RTCPeerConnection {
    ...
    const peer: PeerEntry = { connection: pc };
    ...
    const outgoingTracks = localTracks.filter((track) => track.kind !== 'video');
    ...
    pc.ontrack = (event) => {
      // Faixas recebidas por um RTCRtpTransceiver sem MediaStream associada
      // chegam com `event.streams` vazio (por exemplo, ao compartilhar a tela
      // sem uma câmera local). Monte e reutilize uma stream para a interface.
