...
export default function RoomClient({ roomId }: { roomId: string }) {
  ...
  async function toggleScreenShare() {
    ...
    try {
      ...
      try {
        await manager.replaceVideoTrack(screenTrack, display);
      } catch (err) {
      ...
      if (displayAudioTrack) {
        ...
        // O navegador só entrega áudio quando o tipo de compartilhamento e a
        // opção de áudio foram habilitados no seletor de tela/aba.
        await manager.replaceAudioTrack(cameraStreamRef.current?.getAudioTracks()[0] ?? null).catch(() => {});
        setBanner('Para compartilhar áudio, escolha uma guia do navegador e marque a opção de compartilhar áudio.');
      }
...
function getScreenVideoConstraints(quality: ScreenShareQuality): boolean | MediaTrackConstraints {
  ...
  const profiles: Record<Exclude<ScreenShareQuality, 'auto'>, { width: number; height: number; fps: number }> = {
