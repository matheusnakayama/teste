...
export default function RoomClient({ roomId }: { roomId: string }) {
  ...
  async function toggleScreenShare() {
    ...
    if (sharingScreen) {
      ...
      if (camTrack) camTrack.enabled = camBeforeShareRef.current;
      await manager.replaceVideoTrack(camTrack);
      setSharingScreen(false);
      setCamOn(camBeforeShareRef.current);
      updateLocalParticipant({
        isSharingScreen: false,
        stream: cameraStreamRef.current ?? undefined,
        camOn: camBeforeShareRef.current,
      });
      manager.broadcastScreenShareState(false);
      manager.broadcastMediaState({ micOn, camOn: camBeforeShareRef.current });
      return;
    }
    ...
    try {
      ...
      camBeforeShareRef.current = camOn;
      ...
      screenTrack.onended = () => {
        // Usuário interrompeu o compartilhamento pelo controle nativo do navegador.
        toggleScreenShare();
      };
    } catch (err) {
