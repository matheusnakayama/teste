...
import type { CallError, IceServerConfig, Participant, RoomChatMessage } from '@/lib/types';
...
export default function RoomClient({ roomId }: { roomId: string }) {
  ...
  function sendChatMessage(text: string) {
    ...
    if (!channel || !localIdRef.current) return;
...
function getScreenVideoProfile(
  ...
  const profiles: Record<Exclude<ScreenShareQuality, 'auto'>, VideoSenderProfile & { width: number; height: number }> = {
    '480p30': { width: 854, height: 480, maxFramerate: 30, maxBitrate: 1_000_000 },
    '720p30': { width: 1280, height: 720, maxFramerate: 30, maxBitrate: 2_400_000 },
    '1080p30': { width: 1920, height: 1080, maxFramerate: 30, maxBitrate: 4_000_000 },
    '1080p60': { width: 1920, height: 1080, maxFramerate: 60, maxBitrate: 6_000_000 },
  };
  const profile = profiles[quality];
  return { ...profile, totalBitrate: profile.maxBitrate };
}
...
function normalizeChatMessage(payload: unknown): RoomChatMessage | null {
  if (!payload || typeof payload !== 'object') return null;
  const candidate = payload as Partial<RoomChatMessage>;
  if (typeof candidate.senderId !== 'string' || typeof candidate.text !== 'string') return null;
  ...
  return {
    id: typeof candidate.id === 'string' ? candidate.id.slice(0, 100) : `${candidate.senderId}-${sentAt}`,
    senderId: candidate.senderId.slice(0, 100),
    senderName: typeof candidate.senderName === 'string' ? candidate.senderName.trim().slice(0, 40) || 'Convidado' : 'Convidado',
    text,
    sentAt,
  };
}
